...
export default function App() {
  const { session, signOut, loadData, rolePermissions, hasEnteredApp } = useAppStore();
    ...
    <div className="min-h-screen bg-[#F3F6FA] font-sans selection:bg-blue-200 text-slate-900 overflow-x-hidden">
      ...
      <main className="pb-20 sm:pb-24 max-w-7xl mx-auto min-h-[calc(100vh-60px)] px-2 sm:px-4">
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === 'admin' && session.user.email === '3usajanpapo6@gmail.com' && <Admin />}
            {activeTab === 'nuevo' && <NuevoRegistro />}
            {activeTab === 'fotos' && <RegistroFotos />}
            {activeTab === 'galeria' && <GaleriaFotos />}
            {activeTab === 'perfiles' && <ListaPerfiles />}
            {activeTab === 'historial' && session.user.email === '3usajanpapo6@gmail.com' && <Historial />}
            {activeTab === 'generar' && <VistaGenerar />}
            {/* Fallback de seguridad */}
            {(activeTab === 'admin' || activeTab === 'historial') && session.user.email !== '3usajanpapo6@gmail.com' && (
              <div className="flex flex-col items-center justify-center p-20 text-center space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-slate-500 max-w-xs">Esta sección es para uso exclusivo del Administrador Maestro.</p>
                </div>
              </div>
            )}
          </motion.div>
